            ...
            <tr>
              ...
              <td>
                <div class="menu-name-cell">
                  ...
                  <span class="menu-name">{{ menu.moduleName }}</span>
                </div>
              </td>
              <td class="text-muted text-sm">{{ menu.modulePath || '—' }}</td>
                    ...
                    <table>
                      <tbody>
                        <tr v-for="child in childMenus[menu.id]" :key="child.id" class="child-row">
                          ...
                          <td>
                            <span style="margin-left:20px">└ {{ child.moduleName }}</span>
                          </td>
                          <td style="width:160px" class="text-muted text-sm">{{ child.modulePath || '—' }}</td>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <input v-model="form.moduleName" placeholder="4-6个中文汉字" maxlength="6" />
          </div>
          <div class="form-group">
            <label class="form-label">父菜单</label>
            <select v-model.number="form.parentId">
              <option :value="0">顶级菜单</option>
              <option v-for="m in topLevelMenus" :key="m.id" :value="m.id">{{ m.moduleName }}</option>
            </select>
          </div>
          <div class="form-group">
            <label class="form-label">路由路径</label>
            <input v-model="form.modulePath" placeholder="/system/menu" />
          </div>
          <div class="form-group">
            <label class="form-label">图标名称</label>
            <input v-model="form.moduleIcon" placeholder="el-icon-setting" />
          </div>
          <div class="form-group">
...
import { listModules, searchModules, getModuleChildren, createModule, updateModule, deleteModule } from '@/api/menu'
...
async function onSearch() {
  try {
    menus.value = keyword.value
      ? (await searchModules(keyword.value) || [])
      : (await listModules() || [])
  } catch {
    menus.value = []
  }
...
async function toggleExpand(parentId) {
  if (expandedRows.value.has(parentId)) {
    ...
    try {
      childMenus.value[parentId] = await getModuleChildren(parentId) || []
    } catch {
...
function openCreate() {
  editing.value = false
  form.value = { moduleName: '', parentId: 0, modulePath: '', moduleIcon: '', sortOrder: 0, isVisible: 1 }
  showForm.value = true
}
...
async function saveMenu() {
  ...
  try {
    if (editing.value) {
      await updateModule(form.value.id, {
        moduleName: form.value.moduleName.trim(),
        parentId: form.value.parentId,
        modulePath: form.value.modulePath,
        moduleIcon: form.value.moduleIcon,
...
async function confirmDelete(menu) {
  if (!confirm(`确定删除菜单「${menu.moduleName}」吗？子菜单也会一并删除。`)) return
  try {
    await deleteModule(menu.id)
